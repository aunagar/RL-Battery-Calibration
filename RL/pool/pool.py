...
class Pool(object):
    ...
    def __init__(self, variant):
        ...
        self.memory = {
            ...
            '_s': np.zeros([self.history_horizon + 1, s_dim]),
    ...
    def store(self, s, a, d, raw_d, r, terminal, s_,_s):
        """
        s = current state
        a = predicted action
        d = 
        raw_d = 
        r = reward (cost)
        terminal = is terminal state
        s_ = next state
        _s = previous state
        """
        transition = {'s': s, 'a': a, 'd': d,'raw_d':raw_d, 'r': np.array([r]), 'terminal': np.array([terminal]), 's_': s_,'_s':_s}

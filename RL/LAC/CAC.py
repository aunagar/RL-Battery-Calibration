...
def get_traj():
    ...
    with h5py.File('data/' + 'CMAPPS_Surrogate_' + process+'.h5', 'r') as hdf:
        data = hdf.get('X_') # dim = 16 (Next state)
        X_ = np.array(data)
        data = hdf.get('X')  # 16 (Current State)
        X = np.array(data)
        data = hdf.get('T')  # 1 (Theta)
        T = np.array(data)
        data = hdf.get('W')  # 4 (Next Input)
        W = np.array(data)
...
class CAC(object):
    def __init__(self,
        ...
        with tf.variable_scope('Actor'):
            ...
            self.a, self.deterministic_a, self.a_dist,self.ori_dis = self._build_a(self.S, )  # This network is used to update the parameters in time
    ...
    for i in tqdm(range(50)):
        ...
        for j in range(start_point+1,start_point+1+1000):
            ...
            if ground_traj == []:
                ...
                ground_traj = np.concatenate((ground_traj, traj[j-1,0:16]),axis=0)

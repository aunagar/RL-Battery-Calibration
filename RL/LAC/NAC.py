...
def get_traj():
    process = 'DS229_W_X_m_theta_2D_N_tw_1_stride_10_R_early_100_snr_db_70.0'
...
class NAC(object):
    def __init__(self,
        ...
        with tf.variable_scope('Actor'):
            ...
            log_theta = tf.get_variable('theta', None, tf.float32, initializer=tf.log(theta))
            ...
            _, dna, _, near_dis = self._build_a(self.S_near, reuse=True)
            ...
            self.l_action_noise = tf.reduce_mean(tf.norm(dna - self.deterministic_a))
            ...
            self.adv_train = tf.train.AdamOptimizer(self.LR_A).minimize(-self.l_action_noise, var_list=adv_params)
            ...
            if self.use_lyapunov is True:
                ...
                a_loss = self.labda * self.l_derta + self.alpha * tf.reduce_mean(log_pis) - policy_prior_log_probs + self.beta*self.l_action+ self.theta * self.l_action_noise
            else:
                a_loss = a_preloss
            ...
            if self.use_lyapunov is True:
                self.opt = [self.ltrain, self.lambda_train,self.adv_train]
...
def train(variant):
    ...
    policy = NAC(a_dim,s_dim, policy_params)

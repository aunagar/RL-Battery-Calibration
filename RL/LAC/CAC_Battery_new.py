...
def get_data():
    ...
    train_fraction = 0.8
    data_stack = [np.concatenate((X[i], W[i], X_[i], T[i], state[i]), axis = -1) for i in range(int(0.8*len(X)))]
    return data_stack
...
def BaseNetwork(torch.nn.module):
...
class Actor(BaseNetwork):
    ...
    def __init__(self, input_dim, output_dim, n_layers, layer_sizes, hidden_activation = "relu",
        ...
        for h in layer_sizes:
            model.append(nn.Linear(prev_dim, h))
            model.append(str_to_activation[hidden_activation])
            self.prev_dim = h

        self.model = nn.Sequential(*model).apply(nn.init.normal_)

    def forward(self, states):

        encoder = self.model(states)
        mu = nn.Linear(self.prev_dim, output_dim)(encoder)
        log_sigma = nn.Linear(self.prev_dim, output_dim)(encoder)
    ...
    def sample(self, states):
        ...
        mean, log_sigma = self.forward(states)
        sigma = log_sigma.exp()

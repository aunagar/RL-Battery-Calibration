...
SCALE_beta_MIN_MAX = (0, 10)
SCALE_gamma_MIN_MAX = (0, 10)
...
def get_traj():
    ...
    with h5py.File('data_1/' + 'CMAPPS_Surrogate_' + process+'.h5', 'r') as hdf:
...
class EAC(object):
    def __init__(self,
        ...
        with tf.variable_scope('Actor'):
            ...
            self._S = tf.placeholder(tf.float32, [None, s_dim], '_s')
            self.delta = self._build_adv(self.S)
            self.a_input = tf.placeholder(tf.float32, [None, a_dim], 'a_input')
            self.a_input_ = tf.placeholder(tf.float32, [None, a_dim], 'a_input_')
            self._a_input = tf.placeholder(tf.float32, [None, a_dim], '_a_input')
            ...
            self.gamma = tf.clip_by_value(tf.exp(log_beta), *SCALE_beta_MIN_MAX)
            ...
            adv_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='Actor/Adv')
            ...
            self.S_near=self.S+self.delta
            _, da, _, near_dis= self._build_a(self.S_near, reuse=True)
            _, d_a, _, _ = self._build_a(self._S, reuse=True)
            ...
            if self.use_lyapunov is True:
                ...
                a_loss = self.labda * self.l_derta + self.alpha * tf.reduce_mean(log_pis) - policy_prior_log_probs + self.beta*self.kl_loss
            else:
                a_loss = a_preloss
            ...
            self.diagnotics = [self.labda, self.alpha, self.l_error, tf.reduce_mean(-self.log_pis), self.a_loss,self.beta,self.l_action_out,self.kl_loss,self.distance]

            if self.use_lyapunov is True:
                self.opt = [self.ltrain, self.lambda_train,self.adv_train,self.beta_train]
    ...
    def learn(self, LR_A, LR_C, LR_L,LR_lag, batch):
        ...
        return labda, alpha, l_error, entropy, a_loss,beta,variance,kl,distance
    ...
    def _build_adv(self, s, name='Adv', reuse=None, custom_getter=None):
        ...
        with tf.variable_scope(name, reuse=reuse, custom_getter=custom_getter):
            ...
            return 0.01*tf.layers.dense(net_1, self.s_dim, activation=tf.nn.tanh,trainable=trainable)  # Q(s,a)
    ...
    def _build_l(self, s, a, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('Lyapunov', reuse=reuse, custom_getter=custom_getter):
            ...
            net_1 = tf.layers.dense(net_0, 256, activation=tf.nn.leaky_relu, name='l2', trainable=trainable)  # 原始是30
            net_2 = tf.layers.dense(net_1, 256, activation=tf.nn.leaky_relu, name='l3', trainable=trainable)  # 原始是30
            # net_3 = tf.layers.dense(net_2, 128, activation=tf.nn.leaky_relu, name='43', trainable=trainable)  # 原始是30
...
def train(variant):
    ...
    policy = EAC(a_dim,s_dim, policy_params)
    #policy.restore("log/CMAPSS/CAC-test/0/policy")
        ...
        for j in range(start_point+1,start_point+1+max_ep_steps):
            ...
            if pool.memory_pointer > min_memory_size and global_step % steps_per_cycle == 0:
                ...
                for _ in range(train_per_cycle):
                    ...
                    labda, alpha, l_loss, entropy, a_loss,beta,action_distance,kl,distance = policy.learn(lr_a_now, lr_c_now, lr_l_now, lr_a_now/10, batch)
...
def eval(variant):
    ...
    for i in range(30):
        ...
        for j in tqdm(range(start_point+1,start_point+1+10000)):

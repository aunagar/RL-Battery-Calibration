...
class Battery(gym.Env):
    def __init__(self):
        ...
        ################################# State
        self.high=np.array([1e10,1e10,1e10,1e10,1e10,1e10,1e10,1e10])
        self.action_space = spaces.Box(low=np.array([2.]), high=np.array([12.]), dtype=np.float32)
    ...
    def hover_load(self):
        W = self.payload_weight+self.weight

        p = self.air_density
        At = self.disc_actuator_area

        nh = 0

        P = (W**(3/2))/(nh*((2*p*At)**(1/2)))

        return P

    def climb_load(self,V):
        W = self.payload_weight+self.weight
        p = self.air_density
        At = self.disc_actuator_area


        nc = 0.5493 - 0.01917* math.cos(V*1.127)- 0.02208*math.sin(V*1.127)

        P = W/nc *(V/2 +((V**4)/4 + W/(2*p*At))** (1/2))

        return P

    def decent_load(self,V):
        W = self.payload_weight + self.weight
        p = self.air_density
        At = self.disc_actuator_area

        nd = 0.5591*np.exp(-V*0.1106) -0.03985*np.exp(-2.577*V)

        P = W / nd * (-V / 2 + ((V ** 4) / 4 + W / (2 * p * At)) ** (1 / 2))

        return P

    def horizontal_load(self,V):
        W = self.payload_weight + self.weight
        p = self.air_density
        At = self.disc_actuator_area

        av = 0.07842 + 1.189*V -0.06359 * (V**2) +0.004595 * (V**3)

        v = ((-(V)**2)/2 + (((V)**4)/4 + W/(2*p*At))**2 )**(1/2)

        nhor = 0.5+0.02347*math.cos(V*0.4004)+0.0136*math.sin(V*0.4004)

        P = W / nhor * (V*math.sin(av)+v)

        return P

    def climb(self, V,z_last):
        done = False

        action= self.climb_load(V)
        u = [action]
        x = np.reshape(self.state,(8,1))
        x = self.StateEqn(0, x, u, 0 * self.generateProcessNoise())

        z = self.OutputEqn(0, x, u, 0 * self.generateSensorNoise())
        self.state = np.reshape(x,(8))
        reward_consumption = z[1][0]-z_last


        reward = reward_consumption

        if z[1][0]<=3:
            done = True

        if self.state[7]>self.qSMax*0.99 or self.state[5]>self.qSMax*0.99 or self.state[7]<0.01 or self.state[5]<0.01:
            done = True
            reward =  -1

        return reward,self.state,done,z


    def step(self, action,z_last):
        done = False
...
def test_code():
    ...
    T1, X1, U1, Z1 = B_test.simulate(4500, 1e10)
    ...
    B_test.reset()
    loads = np.array([[8], [10*60], [4], [5*60], [12], [15*60], [5], [20*60], [10], [100*60]])
    T2, X2, U2, Z2 = B_test.simulate(4500, 60,input=loads)
...
def RL_test():
    ...
    for i in range(3150):
        action = [[8]]
        r,s_,done,z=battery.step(action, Z2[0])

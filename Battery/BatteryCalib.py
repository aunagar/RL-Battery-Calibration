...
class Battery:
    ...
    def getDefaultInitialization(self,t0=0,inputself=None, default_load = 8):
        ...
        for i in range(len(self.state)):
            x0[i] = self.state[i]
        # print("state in initialization: ", x0)
    ...
    def applyDegradation(self, qMobile = 7600, Ro = 0.117215):
        ...
        self.Ro = Ro
        # print("state after degradation: ", self.state)
...
class BatteryCalib(gym.Env):
    ...
    def __init__(self):
        ...
        self.low = 0.
        self.high = 1e5
        self.observation_space = spaces.Box(low = np.array([self.low]), 
                                            high = np.array([self.high]))
        self.action_space = spaces.Box(np.array([0.0, 0.0]), np.array([1.0,1.0]))
        self.seed()
        self.model = Battery()
        self.viewer = None
        self.state = None
    ...
    def step(self, action):
        ...
        z_t = last_state[0] # last observation
        z_tp1 = last_state[2] # next observation
...
def test_code():
    ...
    B_test.reset()
    B_test.applyDegradation(5000)
    loads = np.array([[8], [10*60], [4], [5*60], [12], [15*60], [5], [20*60], [10], [10*60]])
    T2, X2, U2, Z2 = B_test.simulate(3150, 1e10)
    # Determine end-of-discharge time for these conditions
